...
public class MyLocation {
    ...
    public boolean getLocation(Context context, LocationResult result)
    {
        ...
        WorkerTask task = new WorkerTask(timer1);
        task.execute();
        return true;
    }

    public class WorkerTask extends AsyncTask<String, Boolean, Boolean>{
    	public Timer timer;
    	public WorkerTask(final Timer timer){
    		this.timer = timer;
    	}
		@Override
    ...
    LocationListener locationListenerNetwork = new LocationListener() {
        public void onLocationChanged(Location location) {
            //timer1.cancel();
            locationResult.gotLocation(location);
            System.out.println("NETWORK LOCATION CHANGED");
            lm.removeUpdates(this);

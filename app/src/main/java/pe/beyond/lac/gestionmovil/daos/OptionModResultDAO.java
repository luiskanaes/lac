...
public class OptionModResultDAO extends BaseDAO {
	public static final String DAO_NAME = "OptionModResultDAO";
	public static final String TB_NAME = "GLS_GR_REL_RESULT_MOD_OPCION";
	public static final String COL_ID = "_id";
	public static final String COL_IDE_RESULTADO_MOD = "IDE_RESULTADO_MOD_MOP";
	public static final String COL_COD_OPCION = "COD_OPCION_OPC";
	public static final String COL_COD_OPCION_MOD = "COD_OPCION_MOD_MOP";
	...
	private final String[][] TB_CREATE_INFO = new String[][] {
			...
			{ COL_IDE_RESULTADO_MOD, Constants.INTEGER },
			{ COL_COD_OPCION, Constants.INTEGER },
			{ COL_COD_OPCION_MOD, Constants.INTEGER },
			{ COL_COD_ESTADO, Constants.INTEGER }};

	public static final int DAO_URI_CODE = 30;

	public static final int INSERT_URI_CODE = 3099;
	...
	public static final int QUERY_ONE_URI_CODE = 3001;
	...
	public static final int QUERY_ALL_URI_CODE = 3002;
	...
	public static final int QUERY_BY_OPTIONID_URI_CODE = 3003;
	public static final String QUERY_BY_OPTIONID_PATH = TB_NAME + Constants.LBL_SLASH
			+ "QueryByOptionId";
	public static final Uri QUERY_BY_OPTIONID_URI = Uri.parse("content://"
			+ Constants.CONFIGURATION_PROVIDER_AUTHORITY + Constants.LBL_SLASH
			+ QUERY_BY_OPTIONID_PATH);
	
	public static final int DELETE_ALL_URI_CODE = 3051;
	...
	public boolean onCreate(final SQLiteDatabase db) {
		...
		db.execSQL(createIndex(TB_NAME, COL_IDE_RESULTADO_MOD));
		return true;
	}
	...
	public int loadDaoUris(final UriMatcher uriMatcher) {
		...
		uriMatcher.addURI(Constants.CONFIGURATION_PROVIDER_AUTHORITY,
				QUERY_BY_OPTIONID_PATH, QUERY_BY_OPTIONID_URI_CODE);
	...
	public Cursor query(Uri uri, Context context, SQLiteDatabase db,
		...
		case QUERY_ONE_URI_CODE:
			selection = " " + TB_NAME + Constants.LBL_DOT + COL_IDE_RESULTADO_MOD
					+ " = ? ";
			break;
		case QUERY_ALL_URI_CODE:
			sortOrder = " " + TB_NAME + Constants.LBL_DOT + COL_IDE_RESULTADO_MOD
					+ " ASC ";
			break;
		case QUERY_BY_OPTIONID_URI_CODE:
	...
	public static ContentValues createContentValues(final Generic entity) {
		OptionModResult sonEntity = (OptionModResult) entity;
		ContentValues values = new ContentValues();
		values.put(COL_IDE_RESULTADO_MOD, sonEntity.getIntOptionModResultId());
		values.put(COL_COD_OPCION, sonEntity.getIntOptionId());
		values.put(COL_COD_OPCION_MOD, sonEntity.getIntOptionModId());
		values.put(COL_COD_ESTADO, sonEntity.getIntState());
		return values;
	}

	public static OptionModResult createObject(Cursor cursor) {
		if (cursor ==null || cursor.getCount()==0){
			return null;
		}
		int intOptionModResultId = cursor.getInt(cursor.getColumnIndex(COL_IDE_RESULTADO_MOD));
		int intOptionId = cursor.getInt(cursor.getColumnIndex(COL_COD_OPCION));
		int intOptionModId = cursor.getInt(cursor.getColumnIndex(COL_COD_OPCION_MOD));
		int intState = cursor.getInt(cursor.getColumnIndex(COL_COD_ESTADO));
		OptionModResult objRegister = new OptionModResult(intOptionModResultId, intOptionId, intOptionModId, intState);

		return objRegister;
	}

	public static List<OptionModResult> createObjects(Cursor cursor) {
		if (cursor ==null || cursor.getCount()==0){
			return null;
		}
		cursor.moveToFirst();
		List<OptionModResult> lst = new ArrayList<OptionModResult>();
		while (!cursor.isAfterLast()) {
			OptionModResult objForm = createObject(cursor);
			lst.add(objForm);
			cursor.moveToNext();
		}
		cursor.close();
